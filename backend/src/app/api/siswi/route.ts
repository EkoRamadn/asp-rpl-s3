...
export async function GET(req: Request) {
    try {
        ...
        if (id !== -1) {
            ...
            if (prm && prm.trim() !== "") {
                const encoded = prm.replace(/,/g, ""); // amanin nilai
            ...
            if (nama_lengkap) query = query.ilike("nama_lengkap", `%${nama_lengkap}%`);
...
export async function PUT(req: Request) {
    try {
        const { id, nis, nama_lengkap, gender, kelas, tanggal_lahir, status_aktif, catatan } = await req.json()
        const { data: siswiData, error: siswiError } = await supabase.from('tbl_siswi').update({
            "nis": nis, "nama_lengkap": nama_lengkap, "gender": gender, "kelas": kelas, "tanggal_lahir": tanggal_lahir, "status_aktif": status_aktif, "catatan": catatan
        }).eq("id", id)
...
export async function POST(req: Request) {
    ...
    interface Siswi {
        nis: number, nama_lengkap: string, gender: string, kelas: string, tanggal_lahir: Date, status_aktif: string, catatan: string
    }
    try {
        ...
        const { data: siswiData, error: siswiError } = await supabase.from('tbl_siswi').insert(dataPac)
...
export async function DELETE(req: Request) {
    try {
        ...
        const { data: siswiData, error: siswiError } = await supabase.from("tbl_siswi")
            .delete()
            .in("id", ids)
        if (siswiError) throw new Error(siswiError.message)
        return NextResponse.json({
